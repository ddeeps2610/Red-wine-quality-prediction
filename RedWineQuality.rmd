...
```{r, echo=FALSE}
qplot(data=wine, fixed.acidity, binwidth=0.5) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data = wine, x=volatile.acidity) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data = wine, x=citric.acid, binwidth=0.05) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data=wine, x=residual.sugar) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data=wine, x=chlorides) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data=wine, x=free.sulfur.dioxide) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data=wine, x=total.sulfur.dioxide) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data=wine, x=pH) + facet_wrap(~quality.factor)
```
...
```{r, echo=FALSE}
qplot(data=wine, quality.factor, sulphates, geom='boxplot') + ylim(0,1)
```
...
```{r, echo=FALSE}
qplot(data=wine, x=sulphates) + facet_wrap(~quality.factor) + scale_y_log10()
```
...
```{r, echo=FALSE}
qplot(data=wine, alcohol) + facet_wrap(~quality.factor) + scale_y_log10()
```
...
grid.arrange(p41,p42,p43,p44,p45,p46,p47,p48,p49,p50,p51, 
             ncol=3, 
             top="Univariate analysis summary")
```
